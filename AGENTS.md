...
- 서브 브리프: 짧게 쓰고, **`[CRITICAL]`** 블록에만 경로·계약·금지·완료 조건을 **전문**으로 둔다. 보고는 간략해도 되나 **변경 파일 전체 경로 목록**은 반드시 남긴다. (선택) 본문 초압축 예시는 [docs/multi-agent.md](docs/multi-agent.md) **서브 브리프 템플릿**의 caveman 절 — **사용자 대화 톤**과 혼동하지 말 것; caveman은 **서브 전달용**이며 사용자가 별도 요청하지 않으면 일반 설명을 유지한다.
