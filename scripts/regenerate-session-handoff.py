# -*- coding: utf-8 -*-
"""Regenerate docs/session-handoff-and-status.md as UTF-8 (no BOM, LF)."""

from pathlib import Path

_DOC = Path(__file__).resolve().parent.parent / "docs" / "session-handoff-and-status.md"


def main() -> None:
	_DOC.parent.mkdir(parents=True, exist_ok=True)
	all_md = "".join(_parts())
	_DOC.write_text(all_md.strip() + "\n", encoding="utf-8")
...
def _p0() -> str:
	return """# 세션 핸드오프 · 상태 요약 (PRD와 별도 문서)
...
**최근 동기화**: 2026-05-12 — **`BankDeposit`** 수기 입금·동액 청구 매칭(`/payments`), **`/reports`** 허브·**`/reports/course-revenue`** 클래스 정산(월 수납+미납), `/p` 납부 이력, `seoulMonthRange` 등. 이전: 2026-05-11 학부모 `/p` 미납·출결·시드; `Payment`·`/makeups` 등.
...
def _p1() -> str:
	return """  - **수납·청구**: 학원 내 **관리자·행정**(`academy_admin`, `office` 등 고권한 스태프).
...
- `npm run seed` 실행 시 학원별 `AcademyMembership` 정리 후 컬렉션은 대략 **`BankDeposit` → `Payment` → `InvoiceLine` → `AttendanceAuditLog` → `Attendance` → `MakeupSession` → `ParentStudentLink` → `Enrollment` → `Course` → `Teacher` → `Student`** 순으로 비우고 샘플을 다시 넣는다.
- 말미에 **`testuser`(`academy_admin`)** 및 **`parent-kim`(`parent`)** 멤버십, `ParentStudentLink` 샘플(김철수 등) 포함.
...
**후속(PR드 대비 미구현·확장)**: 은행 API 실연동, 학부모 영수증·알림, 다학원 런타임(활성 학원 전환) 등.

## 5. 핵심 파일

`src/lib/server/models/{academy,student,teacher,course,enrollment,attendance,attendance-audit-log,invoice-line,payment,bank-deposit,academy-membership,makeup-session,parent-student-link}.ts`, `rbac.ts`, `academy-scope.ts`, 라우트 `src/routes/**`
